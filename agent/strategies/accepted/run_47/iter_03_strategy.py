...
accepted: True
generated: 2026-09-02T23:29:02.408147+00:00
"""
...
def basic_string(draw):
    parts = draw(st.lists(
        st.one_of(
            st.text(alphabet=STRING_TEXT, min_size=0, max_size=16),
            st.sampled_from([
                "\\n", "\\t", "\\\"", "\\\\", "\\u0000",
                "\\u0041", "\\u20ac", "\\U0001f600",
            ]),
...
def multiline_basic_string(draw):
    body = draw(st.text(alphabet=ML_TEXT, min_size=0, max_size=96))
    return '"""' + body + '"""'
...
@composite
def dotted_key(draw):
    parts = draw(st.lists(simple_key(), min_size=2, max_size=6))
    return ".".join(parts)
...
def date_time(draw):
    ...
    zone = draw(st.one_of(
        ...
        st.tuples(
            ...
            st.integers(0, 23),
            st.integers(0, 59),
        ).map(lambda value: value[0] + f"{value[1]:02d}:{value[2]:02d}"),
    ))
...
normal_integer = st.one_of(
    ...
    st.tuples(
        ...
        st.text(alphabet="0123456789abcdefABCDEF_", min_size=1, max_size=24),
...
normal_float = st.one_of(
    ...
    ).map(lambda pair: f"{pair[0]}.{pair[1]:06d}"),
)


extreme_scalar = st.one_of(
    st.text(alphabet=STRING_TEXT, min_size=64, max_size=256).map(
        lambda body: '"' + body + '"'
    ),
    st.text(alphabet=ML_TEXT, min_size=64, max_size=512).map(
        lambda body: '"""' + body + '"""'
    ),
    st.text(alphabet=STRING_TEXT, min_size=64, max_size=256).map(
        lambda body: "'" + body + "'"
...
def expand_value_biased(child):
    ...
    return st.one_of(
        ...
        inline_tables, inline_tables, inline_tables,
        trailing_inline_tables,
        child,
    )
...
def comment_document(draw):
    lines = draw(st.lists(commented_pair(), min_size=1, max_size=6))
    return "\n".join(lines)
