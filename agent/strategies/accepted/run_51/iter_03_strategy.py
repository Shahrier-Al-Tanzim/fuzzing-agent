...
accepted: True
generated: 2026-09-03T16:15:12.702750+00:00
"""
...
basic_key = st.one_of(
    ...
    st.text(
        alphabet=LITERAL_STRING_CHARS, min_size=0, max_size=12
    ).map(lambda s: f"'{s}'"),
)
...
def dotted_key(draw, min_size=1, max_size=5):
    parts = draw(st.lists(basic_key, min_size=min_size, max_size=max_size))
    return ".".join(parts)
...
basic_string = st.one_of(
    st.text(
        alphabet=BASIC_STRING_CHARS, min_size=0, max_size=24
    ).map(lambda s: f'"{s}"'),
    st.sampled_from(
        [
            r'"\n"',
...
literal_string = st.text(
    alphabet=LITERAL_STRING_CHARS, min_size=0, max_size=24
).map(lambda s: f"'{s}'")

multiline_basic = st.text(
    alphabet=BASIC_STRING_CHARS + "\n", min_size=0, max_size=40
).map(lambda s: f'"""{s}"""')

multiline_literal = st.text(
    alphabet=LITERAL_STRING_CHARS + "\n", min_size=0, max_size=40
).map(lambda s: f"'''{s}'''")
...
hex_integer = st.one_of(
    ...
    st.tuples(
        ...
        st.booleans(),
    ).map(lambda p: ("0x" if not p[1] else "0x") + p[0]),
)
...
def inline_table_for(draw, child):
    keys = draw(st.lists(basic_key, min_size=0, max_size=5, unique=True))
...
def unique_pair_document(draw):
    ...
    comments = draw(
        st.lists(
            st.sampled_from(["", " # generated", " # fuzz", " # comment"]),
            min_size=count,
            max_size=count,
        )
    )
    return "\n".join(
        f"{k} = {v}{c}" for k, v, c in zip(keys, values, comments)
    )
...
def biased_document(draw):
    ...
    return "\n".join(f"{k} = {v} # deep" for k, v in zip(keys, values))
...
def deep_array_document(draw):
    depth = draw(st.integers(min_value=49000, max_value=90000))
    leaf = draw(
        st.one_of(
            boolean_value,
            integer_value,
...
def deep_inline_document(draw):
    depth = draw(st.integers(min_value=28000, max_value=52000))
    leaf = draw(st.one_of(basic_string, literal_string, boolean_value))
    quoted = draw(st.booleans())
    opening_parts = []
    for i in range(depth):
        if quoted and i % 2:
            level_key = '"quoted key"'
        elif quoted:
            level_key = "'literal key'"
        else:
            level_key = "a" if i % 2 else "nested"
        opening_parts.append("{")
        opening_parts.append(level_key)
        opening_parts.append(" = ")
    return "deep = " + "".join(opening_parts) + leaf + ("}" * depth)


@composite
def deep_dotted_document(draw):
    depth = draw(st.integers(min_value=32000, max_value=95000))
    quoted = draw(st.booleans())
    if quoted:
        parts = [
            '"quoted key"' if i % 2 else "'literal key'"
            for i in range(depth)
        ]
    else:
        parts = ["a" if i % 2 else "quoted" for i in range(depth)]
    leaf = draw(st.one_of(integer_value, boolean_value, basic_string))
...
def long_flat_document(draw):
    count = draw(st.integers(min_value=14000, max_value=26000))
    ...
    for i in range(count):
        if i % 5 == 0:
            ...
            value_text = f"0x{i:x}"
        lines.append(f"{prefix}{i} = {value_text} # line {i}")
    return "\n".join(lines)


toml_strategy = st.one_of(
    *([ordinary_document()] * 12),
    *([unique_pair_document()] * 8),
    biased_document(),
    duplicate_document(),
    deep_array_document(),
