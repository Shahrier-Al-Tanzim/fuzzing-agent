...
accepted: True
generated: 2026-09-02T21:31:33.269420+00:00
"""
...
basic_key = st.text(
    alphabet=SAFE_BASIC_CHARS.replace('"', "").replace("\\", ""),
    min_size=1,
    max_size=12,
).map(lambda s: '"' + s + '"')

literal_key = st.text(
    alphabet=SAFE_LITERAL_CHARS.replace("'", ""),
    min_size=1,
    max_size=12,
).map(lambda s: "'" + s + "'")
...
def key_value_line(draw, value_strategy=container_value):
    ...
    if draw(st.integers(min_value=0, max_value=4)) == 0:
        c = draw(st.text(alphabet=SAFE_BASIC_CHARS, min_size=0, max_size=12))
        line = line + " #" + c
    return line
...
toml_strategy = st.one_of(
    *([valid_document(container_value)] * 14),
