...
accepted: True
generated: 2026-09-03T13:46:18.760533+00:00
"""
...
quoted_key_strat = st.one_of(basic_string_strat, literal_string_strat)
...
def inline_table_gen(draw, depth=0):
    # 4. Empty inline table explicitly supported
...
def standard_document_gen(draw):
    ...
    keys = [f"k_{i}_{draw(simple_key_strat)}" for i in range(num_lines)]
    lines = []
    for k in keys:
...
# 5. Empty document and comment-only document
...
# Final strategy combination
toml_strategy = st.one_of(
    *([standard_document_gen()] * 15),
