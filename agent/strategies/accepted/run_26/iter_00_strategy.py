...
accepted: True
generated: 2026-08-20T08:22:20.469476+00:00
"""
...
hex_int = st.text(alphabet=HEX_DIGITS, min_size=1, max_size=8).map(lambda h: f"0x{h}")
...
date_str = st.tuples(st.integers(1970, 2099), st.integers(1, 12), st.integers(1, 31)).map(
...
datetime_val = st.one_of(offset_date_time, local_date_time, date_str, time_str)
...
def document(draw):
    ...
    items = draw(st.lists(
        st.one_of(
            key_value_pair(),
            table_header(),
            st.text(alphabet=BASIC_STR_CHARS, max_size=20).map(lambda c: f"# {c}")
        ),
