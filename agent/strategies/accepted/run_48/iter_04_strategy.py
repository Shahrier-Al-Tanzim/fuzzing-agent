...
accepted: True
generated: 2026-09-03T13:18:38.569190+00:00
"""
...
def quoted_key():
    return st.text(alphabet=BASIC_SAFE, min_size=1, max_size=12).map(
        lambda value: '"' + value + '"'
    )
...
@composite
def assignment(draw, values=value_strategy):
...
@composite
def clean_assignment_document(draw, values=value_strategy):
    count = draw(st.integers(min_value=1, max_value=8))
    assignments = [
        draw(assignment(values))
        for _ in range(count)
    ]
    if draw(st.booleans()):
        assignments[0] = assignments[0] + " " + draw(comment())
    return "\n".join(assignments)


@composite
def table_document(draw, values=value_strategy):
    count = draw(st.integers(min_value=1, max_value=6))
    header = draw(key())
    lines = ["table_root = 0", "[" + header + "]"]
    lines.extend(draw(assignment(values)) for _ in range(count))
    if draw(st.booleans()):
...
@composite
def array_table_document(draw, values=value_strategy):
    count = draw(st.integers(min_value=1, max_value=6))
    header = draw(key())
    lines = ["array_root = 0", "[[" + header + "]]"]
    lines.extend(draw(assignment(values)) for _ in range(count))
    return "\n".join(lines)
...
def deep_array_document(draw):
    depth = draw(st.integers(min_value=90000, max_value=115000))
    leaf = draw(
        st.sampled_from(
...
def deep_inline_document(draw):
    depth = draw(st.integers(min_value=85000, max_value=105000))
...
def deep_dotted_document(draw):
    depth = draw(st.integers(min_value=100000, max_value=125000))
    quoted = draw(st.booleans())
    names = []
    for index in range(depth):
        if quoted and index % 2:
...
def many_siblings_document(draw):
    count = draw(st.integers(min_value=60000, max_value=90000))
    value = draw(st.sampled_from(["0", "1", '"x"', "true"]))
    lines = ["k" + str(index) + " = " + value for index in range(count)]
    if draw(st.booleans()):
...
def broad_document(draw):
    ...
    if kind < 5:
        return draw(clean_assignment_document())
    if kind == 5:
        return draw(table_document())
    if kind == 6:
        return draw(array_table_document())
    return draw(duplicate_scoped_document())
