...
accepted: False
generated: 2026-09-02T20:29:41.634295+00:00
"""
...
def key(draw):
    return draw(st.one_of(
        ...
        st.text(alphabet=string.printable.replace("'", "").replace("\\", "").replace("\n", "").replace("\r", ""), min_size=1).map(lambda x: f"'{x}'"),
        st.tuples(st.text(alphabet=UNQUOTED_KEY_CHARS, min_size=1), st.lists(st.text(alphabet=UNQUOTED_KEY_CHARS, min_size=1))).map(lambda x: '.'.join(x))
    ))
