...
accepted: True
generated: 2026-09-03T13:51:57.576285+00:00
"""
...
# Extreme depth direct constructions targeting 60,000 to 100,000 depth (90,000+ target)
@composite
def deep_array_document_gen(draw):
    depth = draw(st.integers(60000, 100000))
...
@composite
def deep_array_table_document_gen(draw):
    depth = draw(st.integers(60000, 100000))
    k = ".".join([f"a{i}" for i in range(depth)])
    val = draw(scalar_val_strat)
    return f"[[{k}]]\nval = {val}"

@composite
def many_siblings_document_gen(draw):
    count = draw(st.integers(2000, 5000))
