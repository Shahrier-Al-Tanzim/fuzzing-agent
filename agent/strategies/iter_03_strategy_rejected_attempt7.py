"""Generated strategy - iteration 3, attempt 7.
accepted: False
generated: 2026-08-15T09:39:23.093705+00:00
"""
...
def value(draw):
    return draw(st.one_of(
        ...
        st.tuples(st.integers(1970, 2100), st.integers(1, 12), st.integers(1, 28))
            .map(lambda t: f"{t[0]:04d}-{t[1]:02d}-{t[2]:02d}T00:00:00.9999999999999999999Z"),
        st.tuples(st.integers(1970, 2100), st.integers(1, 12), st.integers(1, 28))
            .map(lambda t: f"{t[0]:04d}-{t[1]:02d}-{t[2]:02d}T00:00:00.9999999999999999999"),
        st.tuples(st.integers(1970, 2100), st.integers(1, 12), st.integers(1, 28))
        ...
        st.tuples(st.integers(0, 23), st.integers(0, 59), st.integers(0, 59))
            .map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}"),
        st.tuples(st.integers(0, 23), st.integers(0, 59), st.integers(0, 59))
            .map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}.9999999999999999999Z"),
        st.tuples(st.integers(0, 23), st.integers(0, 59), st.integers(0, 59))
            .map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}.9999999999999999999"),
        st.tuples(st.integers(0, 23), st.integers(0, 59), st.integers(0, 59))
            .map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}Z"),
        st.tuples(st.integers(0, 23), st.integers(0, 59), st.integers(0, 59))
            .map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}"),
        st.tuples(st.integers(0x0, 0xFFFFFFFFFFFFFFFF), st.integers(0x0, 0xFFFFFFFFFFFFFFFF))
            .map(lambda t: f"0x{t[0]:x}"),
        st.tuples(st.integers(0o0, 0o7777777777777777777), st.integers(0o0, 0o7777777777777777777))
            .map(lambda t: f"0o{t[0]:o}"),
        st.tuples(st.integers(0b0, 0b1111111111111111111111111111111111111111111111111111111111111111), st.integers(0b0, 0b1111111111111111111111111111111111111111111111111111111111111111))
            .map(lambda t: f"0b{t[0]:b}"),
        st.integers(min_value=0, max_value=2**63-1).map(lambda x: f"0{x}"),
        st.integers(min_value=-2**63, max_value=2**63-1).map(lambda x: f"{x}.9999999999999999999"),
        st.integers(min_value=2**63, max_value=2**64-1).map(str),
        st.text(min_size=1, max_size=10).map(lambda x: f'"{x}"' + '"'),
