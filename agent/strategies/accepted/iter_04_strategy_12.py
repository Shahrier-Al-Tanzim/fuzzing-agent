...
accepted: True
generated: 2026-08-16T18:30:45.600313+00:00
"""
...
@composite
def hex_int(draw):
    return draw(st.text(alphabet="0123456789abcdefABCDEF", min_size=1, max_size=10).map(lambda x: f"0x{x}"))

@composite
def oct_int(draw):
    return draw(st.text(alphabet="01234567", min_size=1, max_size=10).map(lambda x: f"0o{x}"))

@composite
def bin_int(draw):
    return draw(st.text(alphabet="01", min_size=1, max_size=10).map(lambda x: f"0b{x}"))
...
def leading_zero_int(draw):
    return draw(st.text(alphabet="0123456789", min_size=2, max_size=10).map(lambda x: f"0{x}"))

@composite
def fractional_seconds(draw):
    return draw(st.text(alphabet="0123456789", min_size=1, max_size=10).map(lambda x: f".{x}"))

@composite
def int_underscore(draw):
    return draw(st.text(alphabet="0123456789_", min_size=2, max_size=10).map(lambda x: f"{x}"))

@composite
def int_overflow(draw):
    return draw(st.integers(min_value=2**63, max_value=2**64-1).map(str))

toml_strategy = st.one_of(document(), document(),
                          deep_array().map(lambda x: f"deep = {x}"),
                          deep_inline_table().map(lambda x: f"deep = {x}"),
                          deep_dotted_key().map(lambda x: f"{x} = 1"),
                          st.integers(min_value=2**63, max_value=2**64-1).map(lambda x: f"x = {x}"),
                          st.text(alphabet="0123456789", min_size=2, max_size=10).map(lambda x: f"x = 0{x}"),
                          st.text(alphabet="0123456789", min_size=1, max_size=10).map(lambda x: f"x = .{x}"),
                          st.text(alphabet="\\bfnrt", min_size=1, max_size=1).map(lambda x: f"x = {x}"),
                          st.text(alphabet="u", min_size=1, max_size=1).map(lambda x: f"x = {x}XXXX"),
                          st.text(alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ", min_size=1, max_size=1).map(lambda x: f"x = {x}"),
                          st.sampled_from(["inf", "-inf", "nan"]).map(lambda x: f"x = {x}"),
                          st.text(alphabet="0123456789abcdefABCDEF", min_size=1, max_size=10).map(lambda x: f"x = 0x{x}"),
                          st.text(alphabet="01234567", min_size=1, max_size=10).map(lambda x: f"x = 0o{x}"),
                          st.text(alphabet="01", min_size=1, max_size=10).map(lambda x: f"x = 0b{x}"))