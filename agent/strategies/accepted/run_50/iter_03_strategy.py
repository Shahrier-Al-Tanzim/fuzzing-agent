...
accepted: True
generated: 2026-09-03T14:29:56.562501+00:00
"""
    ...
    def extend(child):
        if biased:
            array_elements = st.one_of(
                child, child, child, child, child, scalar_value
            )
...
def ordinary_document(draw):
    ...
    for index in range(count):
        ...
        if kind <= 5:
            ...
            lines.append("quoted" + str(index) + " = " + draw(
                st.one_of(basic_string, literal_string)
            ))
        else:
            lines.append("container" + str(index) + " = " + draw(
                st.one_of(
                    balanced_value,
                    balanced_value,
                    st.just("{}"),
...
def biased_document(draw):
    ...
    for index in range(count):
        ...
        if kind == 0:
            ...
            lines.append("x" + str(index) + " = { a = " + draw(
                biased_value
            ) + ", }")
        else:
            lines.append("x" + str(index) + " = [" + draw(
                biased_value
            ) + ", " + draw(balanced_value) + "]")
    return "\n".join(lines)
...
def deep_array_document(draw):
    depth = draw(st.integers(min_value=50000, max_value=68000))
...
def deep_inline_document(draw):
    depth = draw(st.integers(min_value=52000, max_value=70000))
    quoted = draw(st.booleans())
...
def deep_dotted_document(draw):
    depth = draw(st.integers(min_value=56000, max_value=76000))
    quoted = draw(st.booleans())
    parts = []
    for index in range(depth):
...
def sibling_document(draw):
    count = draw(st.integers(min_value=82000, max_value=95000))
