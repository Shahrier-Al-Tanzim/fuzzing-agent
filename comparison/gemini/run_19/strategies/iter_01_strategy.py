...
accepted: True
generated: 2026-08-19T11:40:01.921238+00:00
"""
from hypothesis import strategies as st
from hypothesis.strategies import composite

UNQUOTED_KEY_CHARS = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_"
NON_ASCII_CHARS = "éàèùâêîôûäëïöüÿçñßαβγδµ日本語中国語🚀"
SAFE_QUOTED_CHARS = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!#$%&'()*+,-./:;<=>?@[]^_`{|}~" + NON_ASCII_CHARS
SAFE_LITERAL_CHARS = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!\"#$%&()*+,-./:;<=>?@[]^_`{|}~" + NON_ASCII_CHARS
...
@composite
def quoted_key(draw):
    is_basic = draw(st.booleans())
    if is_basic:
        s = draw(st.text(alphabet=SAFE_QUOTED_CHARS, min_size=0, max_size=15))
        return f'"{s}"'
    else:
        s = draw(st.text(alphabet=SAFE_LITERAL_CHARS, min_size=0, max_size=15))
        return f"'{s}'"
