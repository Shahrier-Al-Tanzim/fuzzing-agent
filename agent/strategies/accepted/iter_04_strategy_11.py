...
accepted: True
generated: 2026-08-16T18:21:07.481344+00:00
"""
...
def value(draw):
    return draw(st.one_of(
        ...
        st.text(min_size=1, max_size=10).map(lambda x: f"0x{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"0o{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"0b{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"\\u{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"\\U{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"\\{x}"),
        st.text(min_size=1, max_size=10).map(lambda x: f"\\{x}"),
        st.floats(min_value=-1000000, max_value=1000000).map(lambda x: f"{x:.20f}"), # generate over-long fractional seconds
        st.integers(min_value=9223372036854775808, max_value=9223372036854775808).map(str), # generate integers at and past INT64_MAX
        st.text(min_size=1, max_size=10).map(lambda x: f"0{x}"), # generate leading-zero integers
        st.text(min_size=1, max_size=10).map(lambda x: f"inf"), # generate INF
        st.text(min_size=1, max_size=10).map(lambda x: f"nan"), # generate NAN
        st.text(min_size=1, max_size=10).map(lambda x: f"\\u{x}"), # generate unicode escape
        st.text(min_size=1, max_size=10).map(lambda x: f"\\U{x}"), # generate unicode escape
        st.text(min_size=1, max_size=10).map(lambda x: f"\\{x}"), # generate escape sequence
    ))

@composite
def pair(draw):
    return f"{draw(key())} = {draw(st.one_of(value(), array(depth=0), inline_table(depth=0)))}"
...
toml_strategy = st.one_of(document(), document(), deep_array().map(lambda x: f"deep = {x}"), pair_depth_biased())