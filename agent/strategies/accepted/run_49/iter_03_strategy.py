...
accepted: True
generated: 2026-09-03T13:50:02.642075+00:00
"""
...
# 3. Integers with underscores and overflow (Divergence #3 & #4)
...
int_strat = st.one_of(
    ...
    st.sampled_from(["9223372036854775808", "-9223372036854775809", "18446744073709551615"]), # INT64 overflow
    st.sampled_from(["007", "0123", "00001"]), # Leading zero int divergence
...
datetime_strat = st.one_of(
    ...
    st.just("1979-05-27T00:32:00.9999999999999999999Z")  # Divergence #2 (over-long subsecond precision)
)
...
def standard_document_gen(draw):
    num_lines = draw(st.integers(1, 12))
    lines = []
    for i in range(num_lines):
        # Guarantee unique key names per document line to eliminate duplicate key parser rejections
        base_k = f"k{i}_" + draw(unquoted_key_strat)
        is_type = draw(st.sampled_from(["pair", "pair", "pair", "table", "array_table"]))
        comment = draw(st.sampled_from(["", "", draw(comment_text_strat)]))
        if is_type == "pair":
            if draw(st.booleans()):
                k_str = f"{base_k}." + draw(simple_key_strat)
            else:
                k_str = base_k
            v = draw(value_gen())
            lines.append(f"{k_str} = {v}{comment}")
        elif is_type == "table":
            lines.append(f"[{base_k}]{comment}")
        else:
            lines.append(f"[[{base_k}]]{comment}")
    return "\n".join(lines)

# Extreme depth direct constructions (targeting 30,000+ depth to stress stack limits)
@composite
def deep_array_document_gen(draw):
    depth = draw(st.integers(30000, 50000))
...
def many_siblings_document_gen(draw):
    count = draw(st.integers(1000, 3000))
