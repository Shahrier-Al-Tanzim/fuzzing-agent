...
accepted: True
generated: 2026-08-19T06:37:51.857080+00:00
"""
...
quoted_key = st.text(alphabet=SAFE_QUOTED_CHARS, min_size=0, max_size=10).map(lambda s: f'"{s}"')
...
string_scalar = st.one_of(
    ...
    st.text(alphabet=SAFE_QUOTED_CHARS, min_size=0, max_size=15).map(lambda s: f'"""\n{s}\n"""'),
    st.text(alphabet=SAFE_QUOTED_CHARS, min_size=0, max_size=15).map(lambda s: f"'''\n{s}\n'''"),
)
...
def comment_line(draw):
    cmt = draw(st.text(alphabet=SAFE_QUOTED_CHARS, min_size=0, max_size=20))
    return f"#{cmt}"
...
def document(draw):
    ...
    for _ in range(num_entries):
        ...
        if kind == "pair":
            ...
            lines.append(draw(standard_table()))
        elif kind == "array_table":
            lines.append(draw(array_table()))
        else:
