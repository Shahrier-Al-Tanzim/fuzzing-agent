...
accepted: True
generated: 2026-09-01T21:01:43.592809+00:00
"""
from hypothesis import strategies as st
from hypothesis.strategies import composite

BASIC_STR_CHARS = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ !#$%&'()*+,-./:;<=>?@[]^_`{|}~"
LITERAL_STR_CHARS = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ !\"#$%&()*+,-./:;<=>?@[\\]^_`{|}~"
...
@composite
def quoted_key(draw):
    s = draw(st.text(alphabet=BASIC_STR_CHARS, min_size=1, max_size=10))
    return f'"{s}"'
...
def comment_expr(draw):
    comment_text = draw(
        st.text(alphabet=BASIC_STR_CHARS, min_size=0, max_size=20)
    )
...
def document(draw):
    lines = draw(
        st.lists(
            st.one_of(pair_expr(), table_hdr(), comment_expr()),
            min_size=0,
            max_size=15,
        )
    )
