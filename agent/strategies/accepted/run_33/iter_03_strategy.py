...
accepted: True
generated: 2026-08-22T08:43:19.579695+00:00
"""
...
# Key scaling: standard, long, and ultra-long dot chains (100 to 1000 segments)
...
ultra_long_dotted_key_st = st.lists(unquoted_key_st, min_size=100, max_size=1000).map(lambda parts: ".".join(parts))
...
# Shallow recursive structures (for grammar breadth)
@composite
...
# Priority 1: Ultra-deep nesting strategies targeting 5,000 to 35,000 levels
@composite
def ultra_deep_array_val(draw):
    target_depth = draw(st.integers(5000, 35000))
    current = draw(scalar_val)
    brackets_open = "[" * target_depth
    brackets_close = "]" * target_depth
    return f"{brackets_open}{current}{brackets_close}"

@composite
def ultra_deep_inline_table_val(draw):
    target_depth = draw(st.integers(1000, 10000))
    k = draw(unquoted_key_st)
    v = draw(scalar_val)
    prefix = "".join(f"{{{k} = " for _ in range(target_depth))
    suffix = "}" * target_depth
    return f"{prefix}{v}{suffix}"

# Priority 2: Large Table Scope (500 to 3000 key-value pairs)
@composite
def ultra_large_table_document(draw):
    num_entries = draw(st.integers(500, 3000))
    pairs = []
...
# Priority 2: Ultra Long Dot Chains (100 to 1000 segments)
@composite
def long_dot_chain_document(draw):
    num_pairs = draw(st.integers(1, 10))
    lines = []
    for _ in range(num_pairs):
        k = draw(st.one_of(long_dotted_key_st, ultra_long_dotted_key_st))
        v = draw(scalar_val)
        lines.append(f"{k} = {v}")
    return "\n".join(lines)
