...
accepted: True
generated: 2026-09-01T23:02:00.798626+00:00
"""
...
dotted_key = st.lists(simple_key, min_size=2, max_size=3, unique=True).map(
    lambda ks: ".".join(ks)
)
...
oct_int_values = st.sampled_from(
    ["0o755", "0o644", "0o0", "0o1234", "0o777", "0o5_2_1"]
)
hex_int_values = st.sampled_from(
    ["0xDEADBEEF", "0x1_2_3", "0x0", "0xfe00", "0xABCDEF"]
)
bin_int_values = st.sampled_from(
    ["0b110101", "0b0", "0b1_0_1", "0b11111111"]
)
...
other_int_formats = st.sampled_from(["1_000_000", "+123", "0", "-0", "+0"])
...
single_line_str = st.one_of(basic_str, literal_str, escaped_str)
...
all_string_values = st.one_of(single_line_str, ml_basic_str, ml_literal_str)

inline_scalar_value = st.one_of(
    int_values, float_values, bool_values, datetime_values, single_line_str
)

scalar_value = st.one_of(
    int_values, float_values, bool_values, datetime_values, all_string_values
)


@composite
def recursive_value(draw, depth=0, allow_multiline=True):
    if depth >= 3:
        return draw(scalar_value if allow_multiline else inline_scalar_value)
    ...
    if choice == 0:
        ...
        elems = [
            draw(
                recursive_value(
                    depth=depth + 1, allow_multiline=allow_multiline
                )
            )
            for _ in range(n)
        ]
        ...
        n = draw(st.integers(0, 3))
        ks = draw(st.lists(simple_key, min_size=n, max_size=n, unique=True))
        pairs = []
        for k in ks:
            v = draw(recursive_value(depth=depth + 1, allow_multiline=False))
            pairs.append(f"{k} = {v}")
        body = ", ".join(pairs)
...
def document(draw):
    ...
    if draw(st.booleans()):
        cmt = draw(st.text(alphabet=SAFE_BASIC_CHARS, max_size=15))
        lines.append(f"# {cmt}")

    num_top_kv = draw(st.integers(0, 4))
    top_keys = draw(
        st.lists(simple_key, min_size=num_top_kv, max_size=num_top_kv, unique=True)
    )
    for k in top_keys:
        ...
        if draw(st.booleans()):
            cmt = draw(st.text(alphabet=SAFE_BASIC_CHARS, max_size=12))
            lines.append(f"# {cmt}")

    num_sections = draw(st.integers(0, 4))
    section_names = draw(
        st.lists(
            st.integers(1, 100).map(lambda i: f"sec{i}"),
            min_size=num_sections,
            max_size=num_sections,
            unique=True,
        )
    )
    for sec_key in section_names:
        is_array_table = draw(st.booleans())
        header = f"[[{sec_key}]]" if is_array_table else f"[{sec_key}]"
        lines.append(header)

        num_kv = draw(st.integers(0, 4))
        sec_kv_keys = draw(
            st.lists(simple_key, min_size=num_kv, max_size=num_kv, unique=True)
        )
        for k in sec_kv_keys:
            v = draw(recursive_value())
            op = draw(st.sampled_from([" = ", "=", " = "]))
            lines.append(f"{k}{op}{v}")
            if draw(st.booleans()):
