...
accepted: True
generated: 2026-08-16T15:51:49.318163+00:00
"""
...
def dotted_key_value(draw):
    return f"{draw(dotted_key())} = {draw(value())}"
...
def escape_sequence(draw):
    return draw(st.one_of(
        st.text(min_size=1, max_size=1, alphabet='\\'),
        st.text(min_size=1, max_size=1, alphabet='bfnrt'),
        st.text(min_size=1, max_size=1, alphabet='uU')
    ))

@composite
def unicode_escape(draw):
    return draw(st.text(min_size=1, max_size=1, alphabet='uU'))

@composite
def hex_int(draw):
    return draw(st.text(min_size=1, max_size=10, alphabet='0123456789abcdefABCDEF').map(lambda x: f"0x{x}"))

@composite
def oct_int(draw):
    return draw(st.text(min_size=1, max_size=10, alphabet='01234567').map(lambda x: f"0o{x}"))

@composite
def bin_int(draw):
    return draw(st.text(min_size=1, max_size=10, alphabet='01').map(lambda x: f"0b{x}"))

@composite
def leading_zero_int(draw):
    return draw(st.text(min_size=1, max_size=10, alphabet='0123456789').map(lambda x: f"0{x}"))
