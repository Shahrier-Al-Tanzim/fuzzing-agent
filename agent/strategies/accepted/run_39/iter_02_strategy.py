...
accepted: True
generated: 2026-09-01T23:28:56.558204+00:00
"""
...
frac_time_str = st.tuples(
    ...
    st.integers(0, 59),
    st.integers(0, 59),
    st.sampled_from(["999", "9999999999999999999", "12345678901234567890", "123456789"]),
...
@composite
def table_section(draw, header_name):
...
def document(draw):
    ...
    table_names = draw(
        st.lists(simple_key_strategy, min_size=num_tables, max_size=num_tables, unique=True)
    )
    for name in table_names:
        sections.append(draw(table_section(name)))
