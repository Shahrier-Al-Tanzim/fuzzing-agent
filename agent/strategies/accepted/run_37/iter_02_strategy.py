...
accepted: True
generated: 2026-09-01T21:22:36.387456+00:00
"""
...
@composite
def simple_key(draw):
    return draw(st.one_of(unquoted_key(), quoted_key(), literal_quoted_key()))


def make_unique_key(key_str, suffix):
...
def dotted_key(draw, suffix=""):
    ...
    uniq_parts = [
        make_unique_key(p, f"{suffix}_{idx}") for idx, p in enumerate(parts)
    ]
...
def document(draw):
    ...
    lines = []
    current_table_keys = []
    will_duplicate = draw(st.integers(1, 10)) == 1
    ...
    for i in range(num_lines):
        ...
        if line_type == "comment":
            ...
            tbl_name = make_unique_key(base_key, f"tbl_{i}")
            ...
            if will_duplicate and current_table_keys and draw(st.booleans()):
                key_name = draw(st.sampled_from(current_table_keys))
                ...
                if use_dotted:
                    ...
                    k = draw(simple_key())
                    key_name = make_unique_key(k, f"l{i}")
                current_table_keys.append(key_name)
