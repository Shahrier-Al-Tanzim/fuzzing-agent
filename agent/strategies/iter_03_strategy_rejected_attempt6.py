"""Generated strategy - iteration 3, attempt 6.
accepted: False
generated: 2026-09-02T20:39:43.442877+00:00
"""
...
def value(draw):
    return draw(st.one_of(
        ...
        inline_table(),
        st.text(min_size=1, max_size=10).map(lambda x: f'"""{x}"""'),  # Multi-line basic string
        st.text(min_size=1, max_size=10).map(lambda x: f"'''{x}'''"),  # Multi-line literal string
        st.text(alphabet=HEX_CHARS, min_size=1, max_size=10).map(lambda x: f"0x{x}"),  # HEX_INT
        st.text(alphabet=OCT_CHARS, min_size=1, max_size=10).map(lambda x: f"0o{x}"),  # OCT_INT
        st.text(alphabet=BIN_CHARS, min_size=1, max_size=10).map(lambda x: f"0b{x}"),  # BIN_INT
    ))
