...
accepted: True
generated: 2026-09-02T21:39:04.074725+00:00
"""
...
@composite
def dense_inline_table_pairs(draw, value_strategy, allow_trailing):
    ...
    for _ in range(n):
        ...
        parts.append(f"{k} = {v}")
    body = ", ".join(parts)
    if allow_trailing and draw(st.integers(min_value=0, max_value=2)) != 0:
        body = body + ","
    return body


@composite
def extreme_inline_table_pairs(draw, value_strategy, allow_trailing):
    ...
    for _ in range(n):
        ...
        parts.append(f"{k} = {v}")
    body = ", ".join(parts)
    if allow_trailing and draw(st.integers(min_value=0, max_value=4)) != 0:
        body = body + ","
    return body
...
rare_dense_container = st.recursive(
    rare_extreme_scalar,
    lambda children: st.one_of(
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        st.lists(children, min_size=1, max_size=2).map(lambda xs: "[" + ", ".join(xs) + "]"),
        dense_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
        dense_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
        dense_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
    ),
    max_leaves=55,
...
rare_combo_chain_container = st.recursive(
    rare_extreme_scalar,
    lambda children: st.one_of(
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        st.lists(children, min_size=1, max_size=1).map(lambda xs: "[" + ", ".join(xs) + "]"),
        extreme_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
        extreme_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
        extreme_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
        extreme_inline_table_pairs(children, allow_trailing=True).map(lambda body: "{" + body + "}"),
    ),
...
@composite
def extreme_nested_array_of_tables_block(draw):
    outer = draw(key_strategy)
    inner = draw(key_strategy)
    inner2 = draw(key_strategy)
    lines = [f"[[{outer}]]"]
    lines.append(
        "payload = [{ "
        + f"{inner} = [{{ {inner2} = {draw(rare_dense_container)} }}], "
        + f"alt = {{ inner = {draw(rare_dense_container)}, edge = 9223372036854775808, }}, "
        + "}]"
    )
    lines.append(
        "deep = [{ nest = [{ leaf = "
        + draw(rare_dense_container)
        + ", more = { a = 007, b = 1979-05-27T00:32:00.9999999999999999999-07:00, }, }] }]"
    )
    n = draw(st.integers(min_value=1, max_value=4))
    for _ in range(n):
        lines.append(draw(key_value_line(rare_dense_container)))
    return "\n".join(lines)


@composite
def combo_heavy_array_of_tables_block(draw):
    outer = draw(key_strategy)
    k1 = draw(key_strategy)
    k2 = draw(key_strategy)
    k3 = draw(key_strategy)
    k4 = draw(key_strategy)
    lines = [f"[[{outer}]]"]
    lines.append(
        "payload = [{ "
        + f"{k1} = {{ inner = [{{ leaf = {draw(rare_combo_chain_container)} }}], edge = 9223372036854775808, }}, "
        + f"{k2} = [{{ nest = {{ a = 1, b = 2, }}, more = {draw(rare_combo_chain_container)} }}], "
        + "}]"
    )
    lines.append(
        "deep = [{ nest = [{ "
        + f"{k3} = [{{ x = 007 }}, {{ y = 1979-05-27T00:32:00.9999999999999999999-07:00 }}], "
        + f"{k4} = {{ t = 00:32:00.9999999999999999999, u = {draw(rare_combo_chain_container)}, }}, "
        + "}]}]"
    )
    n = draw(st.integers(min_value=1, max_value=4))
    for _ in range(n):
        lines.append(draw(key_value_line(rare_combo_chain_container)))
    return "\n".join(lines)


@composite
def stacked_array_of_tables_document(draw):
    n = draw(st.integers(min_value=2, max_value=5))
    parts = []
    for _ in range(n):
        parts.append(draw(st.one_of(
            combo_heavy_array_of_tables_block(),
            extreme_nested_array_of_tables_block(),
            nested_array_of_tables_block(rare_combo_chain_container),
        )))
    return "\n".join(parts)
...
def rare_combo_document(draw):
    ...
    for _ in range(n):
        parts.append(draw(st.one_of(
            key_value_line(rare_dense_container),
            key_value_line(rare_dense_container),
            standard_table_block(rare_dense_container),
            array_table_block(rare_dense_container),
            nested_array_of_tables_block(rare_dense_container),
            extreme_nested_array_of_tables_block(),
        )))
    return "\n".join(parts)
...
@composite
def combo_heavy_document(draw):
    n = draw(st.integers(min_value=2, max_value=6))
    parts = []
    for _ in range(n):
        parts.append(draw(st.one_of(
            key_value_line(rare_combo_chain_container),
            key_value_line(rare_combo_chain_container),
            key_value_line(rare_combo_chain_container),
            standard_table_block(rare_combo_chain_container),
            array_table_block(rare_combo_chain_container),
            nested_array_of_tables_block(rare_combo_chain_container),
            combo_heavy_array_of_tables_block(),
            extreme_nested_array_of_tables_block(),
        )))
...
def priority_document(draw):
    choice = draw(st.integers(min_value=0, max_value=15))
    if choice == 0:
        return 'x = """hello\nworld"""'
    if choice == 1:
        return 'x = """é\n中\n🙂"""'
    ...
    if choice == 11:
        return '[[a]]\npayload = [{ inner = [{ leaf = 007 }], alt = { t = 1979-05-27T00:32:00.9999999999999999999-07:00, }, }]\ndeep = [{ nest = [{ leaf = { a = 1, b = 2, }, more = [{ x = +inf }] }] }]'
    if choice == 12:
        return 'x = [{ a = { b = [{ c = 9223372036854775808 }], d = 007, }, }]'
    if choice == 13:
        return '[[a]]\npayload = [{ inner = { a = [{ b = { c = 1979-05-27T00:32:00.9999999999999999999-07:00, }, }], d = 007, }, edge = { z = +inf, }, }]\ndeep = [{ nest = [{ leaf = { a = 1, b = 2, }, more = [{ x = 00:32:00.9999999999999999999 }] }] }]'
    if choice == 14:
        return '[[a]]\npayload = [{ inner = [{ leaf = { a = 1, b = 2, }, }], alt = { t = 00:32:00.9999999999999999999, u = 9223372036854775808, }, }]\ndeep = [{ nest = [{ leaf = [{ q = 007 }], more = { r = """é\n中\n🙂""", }, }] }]'
...
def divergence_document(draw):
    ...
    if choice == 6:
        return 'x = """é\n中\n🙂"""'
    if choice == 7:
        return '[[a]]\npayload = [{ inner = { a = 1, b = 2, }, edge = 9223372036854775808, }]\ndeep = [{ nest = [{ leaf = 007, more = { t = 00:32:00.9999999999999999999, }, }] }]'
    if choice == 8:
        return 'x = [{ a = [{ b = { c = 1979-05-27T00:32:00.9999999999999999999-07:00, }, }], }]'
    if choice == 9:
        return '[[a]]\npayload = [{ inner = [{ leaf = { a = 1, b = 2, }, }], alt = { t = 00:32:00.9999999999999999999, u = 9223372036854775808, }, }]\ndeep = [{ nest = [{ leaf = 007, more = { r = 1979-05-27T00:32:00.9999999999999999999-07:00, }, }] }]'
    if choice == 10:
        body = draw(extreme_inline_table_pairs(rare_combo_chain_container, allow_trailing=True))
        return "x = {" + body + "}"
    return "x = 00:32:00"
...
toml_strategy = st.one_of(
    ...
    *([rare_combo_document()] * 8),
    *([combo_heavy_document()] * 10),
    *([stacked_array_of_tables_document()] * 6),
    *([priority_document()] * 4),
    *([divergence_document()] * 4),
