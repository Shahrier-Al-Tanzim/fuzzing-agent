...
accepted: True
generated: 2026-08-21T12:23:33.811060+00:00
"""
...
escapes_strat = st.one_of(
    ...
    st.just('\\U00000020'), st.just('\\U0001F600'), st.just('\\z')
)
...
def duplicate_key_document(draw):
    # Generates accepted TOML documents with repeated keys under DISTINCT tables / array tables
...
def deep_dotted_key_document(draw):
    # Targets key navigation / table insertion routines
...
toml_strategy = st.one_of(
    *[standard_document() for _ in range(20)],
    *[duplicate_key_document() for _ in range(3)],
    empty_document(),
    non_ascii_document(),
    deep_array_document(),
