...
accepted: True
generated: 2026-08-19T20:09:38.371191+00:00
"""
...
def array_table_block(draw):
    ...
    num_entries = draw(st.integers(2, 3))
    lines = []
    for _ in range(num_entries):
        lines.append(f"[[{k}]]")
        num_kvs = draw(st.integers(1, 3))
        sub_keys = draw(st.lists(st.sampled_from(["id", "val", "name", "x", "y"]), min_size=num_kvs, max_size=num_kvs, unique=True))
        for sub_k in sub_keys:
            sub_v = draw(scalar_value)
