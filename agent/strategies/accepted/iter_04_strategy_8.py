...
accepted: True
generated: 2026-08-16T13:36:07.414170+00:00
"""
...
@composite
def pair(draw):
    return f"{draw(key())} = {draw(st.one_of(value(), array(), array_shallow(), inline_table()))}"
...
toml_strategy = st.one_of(
    ...
    ml_literal_string(),
    unicode_escape(),
    st.integers(min_value=0, max_value=2**8-1).map(lambda x: f"0x{x:x}"),
    st.integers(min_value=0, max_value=2**8-1).map(lambda x: f"0o{x:o}"),
    st.integers(min_value=0, max_value=2**8-1).map(lambda x: f"0b{x:b}"),
    st.floats(min_value=-1e10, max_value=1e10).map(lambda x: f"{x:.20f}"),
    st.floats(min_value=-1e10, max_value=1e10).map(lambda x: f"inf" if x > 1e10 else f"-inf" if x < -1e10 else f"nan")
)