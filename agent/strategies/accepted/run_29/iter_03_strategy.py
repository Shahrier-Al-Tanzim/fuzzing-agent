...
accepted: True
generated: 2026-08-21T09:32:47.741014+00:00
"""
...
def deep_toml_value(draw, min_depth=20, max_depth=500):
    ...
    kind = draw(st.sampled_from(["array", "inline", "mixed_inline_array", "trailing_comma_inline"]))
    if kind == "array":
...
def document_builder(draw, deep_mode="none"):
    ...
    for idx in range(num_items):
        ...
        if item_type == "pair":
            ...
            if deep_mode == "extreme":
                v = draw(deep_toml_value(min_depth=50, max_depth=1000))
            elif deep_mode == "moderate":
                v = draw(deep_toml_value(min_depth=10, max_depth=100))
            else:
                v = draw(toml_value())
            ...
            for j in range(num_inner):
                ...
                iv = draw(extreme_value()) if deep_mode == "extreme" else draw(toml_value())
