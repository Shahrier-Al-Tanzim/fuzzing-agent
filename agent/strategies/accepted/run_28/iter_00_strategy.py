...
accepted: True
generated: 2026-08-21T06:27:57.777500+00:00
"""
...
def simple_key(draw):
    ...
    if kind == 0:
        ...
        s = draw(st.text(alphabet=SAFE_STR_CHARS, min_size=0, max_size=10))
        return f'"{s}"'
    else:
        s = draw(st.text(alphabet=SAFE_STR_CHARS, min_size=0, max_size=10))
        return f"'{s}'"
...
def scalar_val(draw):
    ...
    if choice == 0:
        ...
        s = draw(st.text(alphabet=SAFE_STR_CHARS, min_size=0, max_size=15))
        return f'"{s}"'
    elif choice == 10:
        s = draw(st.text(alphabet=SAFE_STR_CHARS, min_size=0, max_size=15))
        return f"'{s}'"
