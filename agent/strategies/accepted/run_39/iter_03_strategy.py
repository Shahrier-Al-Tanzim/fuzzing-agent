...
accepted: True
generated: 2026-09-01T23:35:22.286382+00:00
"""
...
dotted_key_strategy = st.lists(
    simple_key_strategy, min_size=2, max_size=4
...
def inline_table_strategy(draw, depth=0):
    ...
    # Trailing comma in inline table exercises Divergence #1
    trailing = "," if draw(st.booleans()) else ""
...
@composite
def table_header_strategy(draw):
    header_key = draw(key_strategy)
    if draw(st.booleans()):
        return f"[{header_key}]"
    else:
        return f"[[{header_key}]]"
...
def table_section(draw, header_name=None):
    if header_name is None:
        header = draw(table_header_strategy())
    else:
    ...
    local_keys = draw(
        st.lists(
            simple_key_strategy, min_size=num_pairs, max_size=num_pairs, unique=True
        )
    )
...
def flat_document(draw):
    ...
    for _ in range(num_pairs):
        lines.append(draw(pair_strategy()))
        if draw(st.booleans()):
            comment_text = draw(
...
def table_heavy_document(draw):
    ...
    table_names = draw(
        st.lists(
            key_strategy, min_size=num_tables, max_size=num_tables, unique=True
        )
    )
...
def mixed_structure_document(draw):
    # Variety enhancer: mix nested dotted tables, array tables, inline structures, and standalone comments
    sections = []
    if draw(st.booleans()):
    ...
    for _ in range(num_elements):
        ...
        if choice == 1:
            sections.append(draw(pair_strategy()))
        elif choice == 2:
            sections.append(draw(table_section()))
        elif choice == 3:
            # Table with dotted keys inside
            hdr = draw(table_header_strategy())
            k = draw(dotted_key_strategy)
            v = draw(value_strategy())
