...
accepted: True
generated: 2026-09-03T17:00:29.289057+00:00
"""
...
def key(draw):
    ...
    basic = st.text(
        alphabet=QUOTED_KEY_CHARS, min_size=0, max_size=16
    ).map(lambda s: '"' + s + '"')
    literal = st.text(
        alphabet=LITERAL_CHARS, min_size=0, max_size=16
    ).map(lambda s: "'" + s + "'")
...
basic_string = st.one_of(
    st.text(alphabet=BASIC_CHARS, min_size=0, max_size=32).map(
        lambda s: '"' + s + '"'
    ),
    st.tuples(
        st.text(alphabet=BASIC_CHARS, min_size=0, max_size=12),
        st.sampled_from(
            [
                "\\n",
                "\\t",
...
literal_string = st.one_of(
    st.text(alphabet=LITERAL_CHARS, min_size=0, max_size=32).map(
        lambda s: "'" + s + "'"
    ),
    st.sampled_from(
        [
...
oct_int = st.one_of(
    st.sampled_from(
        ["0o0", "0o7", "0o755", "0o777_777", "0o777777777777777777"]
    ),
    st.tuples(
        st.just("0o"),
...
bin_int = st.one_of(
    st.sampled_from(
        ["0b0", "0b1", "0b1010", "0b1111_0000", "0b" + "1" * 64]
    ),
    st.tuples(
        st.just("0b"),
...
date_time = st.one_of(
    ...
    st.tuples(
        ...
        st.integers(0, 59),
        st.integers(0, 999999999999999999999),
    ).map(lambda p: f"{p[0]:02d}:{p[1]:02d}:{p[2]:02d}.{p[3]:021d}"),
    st.sampled_from(
        [
...
def deep_array_document(draw):
    depth = draw(st.integers(min_value=49001, max_value=65000))
...
def deep_inline_document(draw):
    depth = draw(st.integers(min_value=40001, max_value=65000))
    ...
    if style == 0:
        ...
        pieces = [
            ("{'q'=" if i % 3 == 0 else '{"v"=' if i % 3 == 1 else "{a=")
...
def many_sibling_document(draw):
    count = draw(st.integers(min_value=5001, max_value=25000))
    style = draw(st.integers(min_value=0, max_value=2))
    lines = ["[large]"]
