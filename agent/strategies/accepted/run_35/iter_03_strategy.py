...
accepted: True
generated: 2026-09-01T19:36:53.014193+00:00
"""
    ...
    for i in range(num_pairs):
        k = draw(
            st.one_of(
                ...
                st.text(alphabet=SAFE_CHAR, min_size=1, max_size=6).map(
                    lambda s: f'"str_var_{i}_{s}"'
                ),
            )
        )
        v = draw(value_strat)
        comment = f" # comment" if draw(st.booleans()) else ""
...
def array_table_document(draw):
    ...
    for t_idx in range(num_tables):
        tbl_name = draw(
            st.text(alphabet=UNQUOTED_KEY_CHARS, min_size=1, max_size=6).map(
                lambda s: f"tbl_{t_idx}_{s}"
            )
        )
...
def standard_document(draw):
    ...
    for sec_idx in range(num_sections):
        ...
        for _ in range(num_sec_pairs):
            k = draw(key_strat)
            v = draw(value_strat)
            comment = f" # inline comment" if draw(st.booleans()) else ""
            lines.append(f"{k} = {v}{comment}")
