...
accepted: True
generated: 2026-08-21T11:51:53.767241+00:00
"""
...
scalar_values = st.one_of(
    ...
    st.sampled_from([
        ...
        "-9223372036854775809", "18446744073709551615", "007", "000123",
        "0x123abc", "0o755", "0b1010", "0x7fffffffffffffff", "0x8000000000000000",
        "1_000_000", "0x12_ab", "0o7_55", "0b1_01"
    ]),
    st.floats(allow_nan=True, allow_infinity=True).map(str),
    st.sampled_from([
        "inf", "-inf", "+inf", "nan", "-nan", "+nan",
        "0.0", "-0.0", "1e100", "1.5e-10", "1e309", "-1e309", "1e-324", "1_000.000_1"
    ]),
    ...
    st.tuples(
        st.text(alphabet=BASIC_STRING_CHARS, min_size=0, max_size=10),
        st.sampled_from(["\\\n", "\\\r\n"]),
        st.text(alphabet=BASIC_STRING_CHARS, min_size=0, max_size=10)
    ).map(lambda t: f'"""{t[0]}{t[1]}{t[2]}"""'),
    st.text(alphabet=LITERAL_STRING_CHARS + "\n", min_size=0, max_size=20).map(lambda s: f"'''{s}'''"),
    st.tuples(st.integers(0, 9999), st.integers(1, 12), st.integers(1, 28)).map(
    ...
    st.tuples(
        ...
        st.sampled_from(["999999", "9999999999999999999", "1234567890123456789999"])
    ).map(lambda t: f"{t[0]:02d}:{t[1]:02d}:{t[2]:02d}.{t[3]}"),
    st.tuples(
        st.integers(1970, 2100), st.integers(1, 12), st.integers(1, 28),
        st.integers(0, 23), st.integers(0, 59), st.integers(0, 59),
        st.sampled_from(["Z", "+00:00", "-05:00", "+02:30", "+23:59", "-23:59"])
...
def mega_deep_array_doc(draw):
    k = draw(key_strat)
    n = draw(st.integers(min_value=1000000, max_value=1600000))
    return f"{k} = " + ("[" * n) + "1" + ("]" * n)

@composite
def deep_inline_table_doc(draw):
    k = draw(key_strat)
    n = draw(st.integers(min_value=100000, max_value=800000))
    return f"{k} = " + ("{a=" * n) + "1" + ("}" * n)

@composite
def deep_array_of_inline_tables_doc(draw):
    k = draw(key_strat)
    n = draw(st.integers(min_value=50000, max_value=400000))
    return f"{k} = " + ("[{a=" * n) + "1" + ("}]" * n)
