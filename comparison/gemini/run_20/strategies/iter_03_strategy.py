...
accepted: True
generated: 2026-08-19T12:06:37.773730+00:00
"""
...
int_strat = st.one_of(
    ...
    st.sampled_from(["9223372036854775808", "-9223372036854775809", "18446744073709551615", "99999999999999999999", "-99999999999999999999"]),
...
float_strat = st.one_of(
    ...
    st.sampled_from(["inf", "-inf", "+inf", "nan", "-nan", "+nan", "0.0", "-0.0", "1e10", "1.5e-3", "-1.0e+100", "1.7976931348623157e+308", "1e+400"]),
)
...
string_strat = st.one_of(
    ...
    st.sampled_from([
        ...
        '"non_ascii_café"', '"日本語テスト"', '"\u00e9\u00e0\u00e7"', '"\\uFFFF"', '"\\U0001F600"'
    ]),
    st.text(alphabet=BASIC_STRING_SAFE, min_size=0, max_size=20).map(lambda s: f"'{s}'"),
    st.text(alphabet=BASIC_STRING_SAFE, min_size=0, max_size=20).map(lambda s: f'"""{s}"""'),
    st.text(alphabet=BASIC_STRING_SAFE, min_size=0, max_size=20).map(lambda s: f"'''{s}'''"),
...
def deep_array_doc(draw):
    n = draw(st.integers(min_value=1_000, max_value=40_000))
...
def deep_inline_in_array_table_doc(draw):
    depth = draw(st.integers(min_value=15, max_value=150))
