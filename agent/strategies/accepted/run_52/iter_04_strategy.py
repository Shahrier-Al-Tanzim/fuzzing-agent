...
accepted: True
generated: 2026-09-03T17:11:48.770320+00:00
"""
...
KEY_BODY_CHARS = KEY_BODY_CHARS.replace('"', "").replace("\\", "")
BASIC_BODY_CHARS = KEY_BODY_CHARS
LITERAL_BODY_CHARS = KEY_BODY_CHARS.replace("'", "")
ML_BASIC_CHARS = (
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 "
    "_-./:!?@#$%^&*()+={}[];,<>|~'"
).replace('"', "").replace("\\", "")
...
basic_string = st.one_of(
    ...
    st.tuples(
        st.text(alphabet=BASIC_BODY_CHARS, min_size=0, max_size=12),
        st.sampled_from([
            "\\n", "\\t", "\\r", "\\b", "\\f", '\\"', "\\\\", "\\/",
            "\\u0000", "\\u0041", "\\u03a9", "\\U0001f9ea",
        ]),
...
hex_int = st.one_of(
    st.sampled_from([
        "0x0", "0x1", "0x7f", "0x8000000000000000",
        "0xFFFF_FFFF", "0xdead_beef",
        "0xffffffffffffffff", "0xFFFFFFFFFFFFFFFFFFFFFFFF",
    ]),
    st.tuples(
...
floating = st.one_of(
    st.sampled_from([
        "0.0", "-0.0", "+0.0", "1.5", "-3.14159",
        "1e0", "1E+10", "-2e-9", "6.02e23",
        "1_000.000_001", "1.0e+308", "1e-300",
        "inf", "+inf", "-inf", "nan", "+nan", "-nan",
    ]),
    st.tuples(
        st.integers(0, 999999).map(str),
        st.integers(0, 999999999).map(str),
    ).map(lambda p: p[0] + "." + p[1]),
    st.tuples(
        st.integers(0, 999999).map(str),
        st.integers(0, 999999).map(str),
...
def normal_document(draw):
    ...
    if kind <= 5:
        return "\n".join(draw(st.lists(pair, min_size=1, max_size=10)))
    if kind == 6:
        lines = draw(st.lists(
    ...
    if kind == 10:
        return "\n".join(draw(st.lists(
            biased_pair, min_size=1, max_size=5
        )))
    if kind == 11:
        return "strings = " + draw(st.one_of(
            ml_basic_string, ml_literal_string, basic_string
        ))
    if kind == 12:
        return "numbers = " + draw(st.one_of(
            hex_int, oct_int, bin_int, floating
        ))
    header = draw(key())
...
def deep_array_document(draw):
    depth = draw(st.integers(min_value=49001, max_value=76000))
    style = draw(st.integers(0, 4))
    if style == 0:
        opens = "[" * depth
    elif style == 1:
        opens = "[\n" * depth
...
def deep_inline_document(draw):
    depth = draw(st.integers(min_value=42001, max_value=72000))
    style = draw(st.integers(0, 4))
    if style == 0:
        pieces = ["{a="] * depth
    elif style == 1:
        pieces = [
            '{"quoted"=' if i % 2 else "{a="
            for i in range(depth)
        ]
    elif style == 2:
        pieces = [
...
def deep_dotted_document(draw):
    depth = draw(st.integers(min_value=42001, max_value=76000))
    style = draw(st.integers(0, 4))
    if style == 0:
        pieces = ["a"] * depth
    elif style == 1:
        ...
        pieces = [
            '"escaped-like"' if i % 2 else "'literal segment'"
            for i in range(depth)
        ]
...
def many_sibling_document(draw):
    count = draw(st.integers(min_value=36001, max_value=54000))
...
def long_document(draw):
    count = draw(st.integers(min_value=22000, max_value=40000))
    style = draw(st.integers(0, 3))
    lines = []
    for i in range(count):
        if style == 0:
...
def special_document(draw):
    ...
    if kind == 5:
        return "escape_value = " + draw(st.sampled_from([
            '"\\n\\t\\r\\b\\f"',
            '"\\u0000\\u0041\\u03a9"',
            '"\\U0001f9ea"',
            '"\\\\\\/\\\""',
    ...
    if kind == 6:
        return "time_value = " + draw(date_time)
    header = draw(key())
    body = draw(st.lists(pair, min_size=1, max_size=4))
    return "[[" + header + "]]\n" + "\n".join(body)
