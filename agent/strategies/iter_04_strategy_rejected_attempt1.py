...
accepted: False
generated: 2026-09-01T23:00:33.578733+00:00
"""
...
def document(draw):
    ...
    # Top-level key-values without any table header (scoped to avoid collisions)
    ...
    for sec_idx in range(num_sections):
        sec_key = f"sec_{sec_idx}"
        is_array_table = draw(st.booleans())

        if draw(st.booleans()):
            sec_key = f"sec_{sec_idx}.sub_{sec_idx}"
        ...
        for kv_idx in range(num_kv):
            k = f"item_{kv_idx}"
            if draw(st.booleans()):
                raw_k = draw(simple_key)
                k = (
                    f"s_{kv_idx}_{raw_k}"
                    if not (raw_k.startswith('"') or raw_k.startswith("'"))
                    else f'"s_{kv_idx}_' + raw_k[1:]
                )
            v = draw(recursive_value())
            op = draw(st.sampled_from([" = ", "=", " = "]))
            lines.append(f"{k}{op}{v}")
