...
accepted: True
generated: 2026-09-01T21:35:05.231223+00:00
"""
...
@composite
def document(draw):
    # Structural variety: select layout pattern to produce diverse shapes
    doc_style = draw(st.sampled_from(["interleaved", "sectioned", "dotted_heavy"]))

    lines = []
    global_table_names = set()
    current_table_keys = set()

    if doc_style == "sectioned":
        # Root pairs followed by structured table sections
        num_root = draw(st.integers(0, 4))
        for i in range(num_root):
            k = draw(simple_key())
            v = draw(value_strat(max_depth=1))
            lines.append(f"{k} = {v}")
        ...
        for s in range(num_sections):
            ...
            header_key = draw(table_header_key())
            if header_key in global_table_names and tbl_kind == "standard":
                header_key = make_key_with_suffix(header_key, f"sec_{s}")
            global_table_names.add(header_key)
            ...
            for p in range(num_pairs):
                k = draw(simple_key())
                if k in sec_keys:
                    k = make_key_with_suffix(k, f"p_{p}")
                sec_keys.add(k)
                ...
                lines.append(f"{k} = {v}{comment_suffix}")

    elif doc_style == "dotted_heavy":
        # Heavy use of nested dotted keys without table headers
        num_pairs = draw(st.integers(3, 8))
        for i in range(num_pairs):
            k = draw(dotted_key(suffix=f"d{i}"))
            v = draw(value_strat(max_depth=2))
            lines.append(f"{k} = {v}")

    else:
        # Dynamic interleaved document line-by-line
        num_lines = draw(st.integers(min_value=3, max_value=12))
        for i in range(num_lines):
            line_type = draw(
                st.sampled_from(
                    ["pair", "dotted_pair", "table", "array_table", "comment", "blank"]
                )
            )
            ...
            if line_type == "blank":
            ...
            elif line_type in ("table", "array_table"):
                base_key = draw(table_header_key())
                tbl_name = base_key
                if tbl_name in global_table_names and line_type == "table":
                    tbl_name = make_key_with_suffix(tbl_name, f"tbl_{i}")
                global_table_names.add(tbl_name)
                ...
                lines.append(f"{k}{sep}{v}{comment_suffix}")
