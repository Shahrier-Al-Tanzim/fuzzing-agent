...
accepted: True
generated: 2026-08-20T06:38:17.884632+00:00
"""
...
def document(draw):
  ...
  for s in range(num_sections):
    if s > 0 or draw(st.booleans()):
      tbl_name = f"sec{s}"
      if draw(st.booleans()):
        tbl_name = f"{tbl_name}.sub{draw(st.integers(1, 5))}"
    ...
    keys = draw(
        st.lists(unquoted_key, min_size=num_entries, max_size=num_entries, unique=True)
    )
...
def deep_accepted_array(draw):
  n = draw(st.integers(min_value=5000, max_value=35000))
...
def deep_accepted_mixed(draw):
  n = draw(st.integers(min_value=2500, max_value=17500))
  val = draw(scalar_value)
  return f"v = {'[{a=' * n}{val}{'}]' * n}"
