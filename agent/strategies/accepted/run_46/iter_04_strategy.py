...
accepted: True
generated: 2026-09-02T23:15:27.872996+00:00
"""
...
BASIC_KEY_SAFE = BASIC_SAFE.replace('"', "").replace("\\", "").replace("\n", "").replace("\r", "")
LITERAL_SAFE = BASIC_SAFE.replace("'", "").replace("\n", "").replace("\r", "")
...
def ml_basic_string(draw):
    ...
    suffix = draw(
        st.sampled_from(
            ["", "text", "unicode \U0001f600", "multiple\nlines\nof text"]
        )
    )
    return '"""' + body + "\n" + suffix + '"""'
...
def key(draw):
    return draw(
        st.one_of(
            ...
            st.text(
                alphabet=BASIC_KEY_SAFE, min_size=1, max_size=14
            ).map(lambda text: '"' + text + '"'),
            st.text(
                alphabet=LITERAL_SAFE, min_size=1, max_size=14
            ).map(lambda text: "'" + text + "'"),
        )
    )
...
integer = st.one_of(
    ...
    st.integers(0, 10**18).map(lambda value: str(value) + "_000"),
...
floating = st.one_of(
    ...
    st.tuples(
        ...
        st.integers(0, 999999999).map(lambda value: str(value).zfill(9)),
...
date_time = st.one_of(
    st.tuples(
        st.integers(1970, 2100).map(lambda value: str(value).zfill(4)),
        st.integers(1, 12).map(lambda value: str(value).zfill(2)),
        st.integers(1, 28).map(lambda value: str(value).zfill(2)),
        st.integers(0, 23).map(lambda value: str(value).zfill(2)),
        st.integers(0, 59).map(lambda value: str(value).zfill(2)),
        st.integers(0, 59).map(lambda value: str(value).zfill(2)),
