...
accepted: True
generated: 2026-08-21T12:26:59.592791+00:00
"""
...
@composite
def inline_table_with_escapes_document(draw):
    k = draw(key_strat)
    num_kvs = draw(st.integers(3, 8))
    kvs = []
    for i in range(num_kvs):
        ik = draw(unquoted_key_strat)
        iv = draw(st.one_of(basic_string_strat, ml_basic_string_strat, date_time_value_strat))
        kvs.append(f"{ik} = {iv}")
    trailing = draw(st.one_of(st.just(""), st.just(", ")))
    return f"{k} = {{ " + ", ".join(kvs) + trailing + " }}"
...
toml_strategy = st.one_of(
    ...
    *[inline_table_with_escapes_document() for _ in range(3)],
