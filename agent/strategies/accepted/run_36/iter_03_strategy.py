...
accepted: True
generated: 2026-09-01T20:45:44.039691+00:00
"""
...
def document(draw):
    ...
    if draw(st.booleans()):
        doc_lines.append(f"# {draw(unquoted_key_st)} document")

    if draw(st.booleans()):
        num_kv = draw(st.integers(1, 4))
        for i in range(num_kv):
            k_base = draw(st.sampled_from(["simple", "quoted", "dotted"]))
            if k_base == "simple":
                k = f"root_{i}_" + draw(unquoted_key_st)
            elif k_base == "quoted":
                q = draw(
                    st.text(alphabet=BASIC_STR_ALPHABET, min_size=1, max_size=8)
                )
                k = f'"root_{i}_{q}"'
            else:
                k = f"root_{i}." + draw(unquoted_key_st)
            v = draw(value_strategy())
            cmt = (
                f" # {draw(st.text(alphabet=BASIC_STR_ALPHABET, min_size=0, max_size=10))}"
                if draw(st.booleans())
                else ""
            )
    ...
    for sec_idx in range(num_sections):
        ...
        if sec_type == "standard":
            ...
            sub_k = draw(unquoted_key_st)
            header = f"[{sec_name}.{sub_k}]"
        elif sec_type == "dotted_array":
            sub_k = draw(unquoted_key_st)
            header = f"[[{sec_name}.{sub_k}]]"
        else:
            header = f'["{sec_name}"]'
        ...
        if draw(st.booleans()):
            doc_lines.append(f"# Header comment {sec_idx}")
        ...
        for i in range(num_kv):
            ...
            if k_choice == 1:
                k = f"k_{sec_idx}_{i}_" + draw(unquoted_key_st)
            elif k_choice == 2:
                q = draw(
                    st.text(alphabet=BASIC_STR_ALPHABET, min_size=1, max_size=8)
                )
                k = f'"key_{sec_idx}_{i}_{q}"'
            else:
                k = f"sub_{sec_idx}_{i}." + draw(unquoted_key_st)
