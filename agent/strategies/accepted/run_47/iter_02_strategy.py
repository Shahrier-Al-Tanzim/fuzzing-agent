...
accepted: True
generated: 2026-09-02T23:27:32.623076+00:00
"""
...
def expand_value(child):
    array_element = st.one_of(child, child, child, child, child)
    arrays = st.lists(array_element, min_size=0, max_size=4).map(
...
def expand_value_biased(child):
    array_element = st.one_of(child, child, child, child, child, child, child)
    arrays = st.lists(array_element, min_size=1, max_size=2).map(
        lambda values: "[" + ", ".join(values) + "]"
    )
...
def biased_document(draw):
    ...
    lines = [draw(biased_pair()) for _ in range(pair_count)]
