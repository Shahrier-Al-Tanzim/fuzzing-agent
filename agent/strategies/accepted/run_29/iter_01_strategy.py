...
accepted: True
generated: 2026-08-21T09:27:08.186907+00:00
"""
...
def safe_key(draw, prefix=""):
    ...
    if kind == "unquoted":
        ...
        base = draw(st.text(alphabet=BASIC_STR_CHARS + NON_ASCII_CHARS, min_size=0, max_size=10))
...
ml_basic_str = st.text(alphabet=BASIC_STR_CHARS + NON_ASCII_CHARS + "\n ", min_size=0, max_size=20).map(lambda s: f'"""{s}"""')
ml_literal_str = st.text(alphabet=LITERAL_STR_CHARS + NON_ASCII_CHARS + "\n ", min_size=0, max_size=20).map(lambda s: f"'''{s}'''")
...
@composite
def deep_toml_value(draw, depth=0):
    if depth >= 220:
        return draw(scalar_value)
    
    choice = draw(st.sampled_from(["array", "array", "array", "inline", "inline", "scalar"]))
    if choice == "array":
        elem = draw(deep_toml_value(depth=depth + 1))
        return f"[{elem}]"
    elif choice == "inline":
        k = f"dk{depth}_" + draw(unquoted_key_strat)
        v = draw(deep_toml_value(depth=depth + 1))
        return f"{{{k} = {v}}}"
    else:
        return draw(scalar_value)
