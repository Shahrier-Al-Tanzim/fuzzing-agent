"""Generated strategy - iteration 3, attempt 3.
accepted: False
generated: 2026-09-02T20:39:14.935740+00:00
"""
...
ESCAPE_SEQUENCES = ['\\n', '\\t', '\\"', '\\\'', '\\\\', '\\b', '\\f', '\\r']
UNICODE_ESCAPE = [f'\\u{hex(i)[2:].zfill(4)}' for i in range(0x10000)]
...
def escape_sequence(draw):
    return draw(st.one_of(st.sampled_from(ESCAPE_SEQUENCES), st.sampled_from(UNICODE_ESCAPE)))
...
def value(draw):
    return draw(st.one_of(
        ...
        st.just('true'),
        st.just('false'),
        st.lists(escape_sequence()).map(lambda lst: f'"{"".join(lst)}"'),  # Basic string with escape sequences
        st.text(min_size=1).map(lambda x: f'"{x}"'),
        st.text(min_size=1).map(lambda x: f"'{x}'"),
        st.lists(st.one_of(value(), inline_table())).map(lambda lst: f"[{', '.join(lst)}]"),
        inline_table(),
