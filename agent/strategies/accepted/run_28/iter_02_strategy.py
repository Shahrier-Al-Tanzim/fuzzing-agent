...
accepted: True
generated: 2026-08-21T06:41:09.714164+00:00
"""
...
def document(draw):
    ...
    for section_idx in range(num_sections):
        if section_idx > 0:
            ...
            if hdr not in used_tables or draw(st.integers(1, 20)) == 1:
                used_tables.add(hdr)
                lines.append(hdr)
        ...
        for _ in range(num_pairs):
            k = draw(simple_key())
            # Occasionally emit duplicate key to test parser rejection paths
            if k not in used_keys or draw(st.integers(1, 20)) == 1:
                used_keys.add(k)
                v = draw(value_strat())
                lines.append(f"{k} = {v}")
    return "\n".join(lines)
