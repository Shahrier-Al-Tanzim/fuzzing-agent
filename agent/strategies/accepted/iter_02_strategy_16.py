...
accepted: True
generated: 2026-08-19T07:15:08.769509+00:00
"""
"""Generated strategy - iteration 2.
Focus:
- Push into rare combinations: deeply nested inline tables inside array-of-tables.
- Extreme numeric / string values in nested positions.
- Eliminate key collisions across document sections to reduce 'key exists' rejections.
"""
...
@composite
def nested_inline_table(draw, max_depth=4):
    if max_depth <= 0:
        return draw(scalar_val)

    choice = draw(st.sampled_from(["inline_table", "array", "scalar"]))
    if choice == "scalar":
        return draw(scalar_val)
    elif choice == "array":
...
def array_table_with_nested_inline(draw):
    ...
    for _ in range(num_tables):
        ...
        for k_idx in range(num_keys):
            k = f"field_{k_idx}"
            v = draw(nested_inline_table(max_depth=4))
            lines.append(f"{k} = {v}")
    return "\n".join(lines)
    ...
    for idx in range(num_entries):
        ...
        if kind == "pair":
            ...
            for _ in range(draw(st.integers(1, 2))):
                ...
                for p_idx in range(num_pairs):
                    pk = draw(unquoted_key)
                    pv = draw(nested_inline_table(max_depth=3))
                    lines.append(f"{pk}_{p_idx} = {pv}")
        else:
